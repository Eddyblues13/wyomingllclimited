        ...
        <div class="flex items-center justify-between px-4 py-3">
            <div class="flex items-center gap-3">
                ...
                <div class="flex items-center gap-2">
                    <div class="w-8 h-8 bg-gradient-to-br from-orange to-yellow-500 rounded-lg flex items-center justify-center shrink-0">
    ...
    <div class="flex min-h-[calc(100vh-57px)]">
        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-gray-800 border-r border-gray-700 transition-transform duration-300 lg:translate-x-0 lg:static lg:inset-0 pt-14 lg:pt-0 shrink-0"
            ...
            <nav class="p-4 space-y-1 mt-4">
                ...
                <a href="{{ route('admin.companies') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg bg-orange/10 text-orange border border-orange/20">
        ...
        <main class="flex-1 min-w-0 p-4 lg:p-8">
            ...
            <div class="mb-6">
                <h1 class="text-2xl lg:text-3xl font-bold text-white">Manage Companies</h1>
                <p class="text-gray-400 text-sm mt-1">{{ $companies->total() }} total companies</p>
            </div>
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="border-b border-gray-700 bg-gray-900/40">
                                ...
                                <th class="text-left py-3 px-4 text-gray-400 text-xs font-medium uppercase hidden md:table-cell">Owner</th>
                                <th class="text-left py-3 px-4 text-gray-400 text-xs font-medium uppercase hidden lg:table-cell">Type</th>
                                <th class="text-left py-3 px-4 text-gray-400 text-xs font-medium uppercase hidden xl:table-cell">State</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-gray-700/50 hover:bg-gray-700/30">
                                <td class="py-3 px-4">
                                    <p class="text-white text-sm font-medium truncate max-w-[160px] lg:max-w-xs">{{ $company->company_name }}</p>
                                    <p class="text-gray-500 text-xs md:hidden">{{ $company->user->name ?? 'N/A' }}</p>
                                </td>
                                <td class="py-3 px-4 text-gray-400 text-sm hidden md:table-cell">
                                    <span class="truncate block max-w-[130px]">{{ $company->user->name ?? 'N/A' }}</span>
                                </td>
                                <td class="py-3 px-4 text-gray-400 text-sm hidden lg:table-cell whitespace-nowrap">{{ $company->entity_type_label }}</td>
                                <td class="py-3 px-4 text-gray-400 text-sm hidden xl:table-cell whitespace-nowrap">{{ $company->formation_state }}</td>
                                <td class="py-3 px-4">
                                    <form method="POST" action="{{ route('admin.companies.status', $company->id) }}">
                                        @csrf @method('PATCH')
                                        <select name="status" onchange="this.form.submit()"
                                            class="bg-gray-700 border border-gray-600 text-xs rounded-lg px-2 py-1.5 text-white focus:ring-orange focus:border-orange w-full sm:w-auto
                                ...
                                <td class="py-3 px-4 text-gray-500 text-sm hidden lg:table-cell whitespace-nowrap">{{ $company->created_at->format('M d, Y') }}</td>
                                <td class="py-3 px-4">
                            ...
                            @empty
                            <tr><td colspan="7" class="py-10 text-center text-gray-500">No companies found</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                @if($companies->hasPages())
                <div class="p-4 border-t border-gray-700">{{ $companies->links() }}</div>
                @endif
            </div>
            ...
            <div class="sm:hidden space-y-3">
                ...
                <div class="bg-gray-800 rounded-xl border border-gray-700 p-4">
                    <div class="mb-3">
                        <p class="text-white font-semibold truncate">{{ $company->company_name }}</p>
                        <p class="text-gray-400 text-xs mt-0.5">{{ $company->user->name ?? 'N/A' }}</p>
                    </div>
                    <div class="grid grid-cols-2 gap-2 text-xs mb-3">
                        <div class="bg-gray-700/40 rounded-lg p-2">
                            <p class="text-gray-500 mb-0.5">Type</p>
                            <p class="text-white font-medium">{{ $company->entity_type_label }}</p>
                        </div>
                        <div class="bg-gray-700/40 rounded-lg p-2">
                            <p class="text-gray-500 mb-0.5">State</p>
                            <p class="text-white font-medium">{{ $company->formation_state }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3 pt-2 border-t border-gray-700">
                        <form method="POST" action="{{ route('admin.companies.status', $company->id) }}" class="flex-1">
                            @csrf @method('PATCH')
                            <select name="status" onchange="this.form.submit()"
                                class="w-full bg-gray-700 border border-gray-600 text-xs rounded-lg px-2 py-2 text-white focus:ring-orange focus:border-orange">
                                <option value="pending" @selected($company->status === 'pending')>Pending</option>
                                <option value="processing" @selected($company->status === 'processing')>Processing</option>
                                <option value="approved" @selected($company->status === 'approved')>Approved</option>
                                <option value="rejected" @selected($company->status === 'rejected')>Rejected</option>
                        ...
                        <form method="POST" action="{{ route('admin.companies.delete', $company->id) }}" onsubmit="return confirm('Delete this company?')">
                            @csrf @method('DELETE')
                            <button type="submit" class="text-red-400 hover:text-red-300 transition text-sm px-3 py-2 bg-red-500/10 rounded-lg">Delete</button>
                        </form>
                    </div>
                </div>
                @empty
                <div class="bg-gray-800 rounded-xl border border-gray-700 p-10 text-center text-gray-500">No companies found</div>
                @endforelse
                @if($companies->hasPages())
                <div class="pt-2">{{ $companies->links() }}</div>
                @endif
            </div>
        </main>
    </div>
