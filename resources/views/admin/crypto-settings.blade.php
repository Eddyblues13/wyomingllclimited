    ...
    <div class="flex min-h-[calc(100vh-57px)]">
        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-gray-800 border-r border-gray-700 transition-transform duration-300 lg:translate-x-0 lg:static lg:inset-0 pt-14 lg:pt-0 shrink-0"
            ...
            <nav class="p-4 space-y-1 mt-4">
                ...
                <a href="{{ route('admin.crypto-settings') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg bg-orange/10 text-orange border border-orange/20">
        ...
        <main class="flex-1 min-w-0 p-4 lg:p-8">
            ...
            <div class="mb-6">
                <h1 class="text-2xl lg:text-3xl font-bold text-white">Crypto Deposit Settings</h1>
                <p class="text-gray-400 text-sm mt-1">Update the deposit addresses for all supported cryptocurrency networks.</p>
            </div>
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="border-b border-gray-700 bg-gray-900/50">
                                    <th class="text-left py-4 px-5 text-gray-400 text-xs font-semibold uppercase">Asset</th>
                                    <th class="text-left py-4 px-5 text-gray-400 text-xs font-semibold uppercase">Network</th>
                                    <th class="text-left py-4 px-5 text-gray-400 text-xs font-semibold uppercase">Deposit Address</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-700/50">
                                ...
                                <tr class="hover:bg-gray-700/20 transition-colors">
                                    <td class="py-4 px-5">
                                        <div class="flex items-center gap-3">
                                            <div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0" style="background-color: {{ $crypto->bg_color ?? '#2b3648' }};">
                                                <img src="{{ $crypto->icon_url }}" alt="{{ $crypto->name }}" class="w-5 h-5 object-contain">
                                            </div>
                                            <div>
                                                <span class="font-bold text-white text-sm uppercase block">{{ $crypto->name }}</span>
                                                <span class="text-gray-400 text-xs uppercase">{{ $crypto->symbol }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="py-4 px-5">
                                        <span class="inline-block px-2.5 py-1 text-xs rounded-full font-bold uppercase bg-gray-700 border border-gray-600 text-orange whitespace-nowrap">{{ $crypto->network }}</span>
                                    </td>
                                    <td class="py-4 px-5">
                                        <input type="text" name="addresses[{{ $crypto->id }}]" value="{{ $crypto->deposit_address }}"
                                ...
                                @empty
                                <tr><td colspan="3" class="py-10 text-center text-gray-500">No coins configured. Please run the seeder.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div class="md:hidden divide-y divide-gray-700/50">
                        ...
                        <div class="p-4">
                            <div class="flex items-center gap-3 mb-3">
                                <div class="w-9 h-9 rounded-full flex items-center justify-center shrink-0" style="background-color: {{ $crypto->bg_color ?? '#2b3648' }};">
                                    <img src="{{ $crypto->icon_url }}" alt="{{ $crypto->name }}" class="w-5 h-5 object-contain">
                                </div>
                                <div class="min-w-0 flex-1">
                                    <span class="font-bold text-white text-sm uppercase block">{{ $crypto->name }}</span>
                                    <span class="inline-block px-2 py-0.5 text-xs rounded-full font-bold uppercase bg-gray-700 border border-gray-600 text-orange mt-0.5">{{ $crypto->network }}</span>
                                </div>
                            </div>
                            <input type="text" name="addresses[{{ $crypto->id }}]" value="{{ $crypto->deposit_address }}"
                                   placeholder="Enter {{ $crypto->symbol }} deposit address"
                                   class="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2.5 text-white font-mono text-xs focus:outline-none focus:ring-2 focus:ring-orange focus:border-orange">
                        </div>
                        @empty
                        <div class="p-10 text-center text-gray-500">No coins configured.</div>
                        @endforelse
                    </div>

                    <div class="bg-gray-900/50 border-t border-gray-700 p-4 lg:p-6 flex justify-end">
                        <button type="submit" class="w-full sm:w-auto bg-orange hover:bg-orange-600 text-white font-semibold py-3 px-8 rounded-xl shadow-lg transition-all active:scale-95">
                            Save Settings
                        </button>
                    </div>
                </div>
