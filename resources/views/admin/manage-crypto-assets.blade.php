    ...
    <div class="flex min-h-[calc(100vh-57px)]">
        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-gray-800 border-r border-gray-700 transition-transform duration-300 lg:translate-x-0 lg:static lg:inset-0 pt-14 lg:pt-0 shrink-0"
            ...
            <nav class="p-4 space-y-1 mt-4">
                ...
                <a href="{{ route('admin.manage-crypto') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg bg-orange/10 text-orange border border-orange/20">
        ...
        <!-- Main -->
        <main class="flex-1 min-w-0 p-4 lg:p-8" x-data="cryptoManager()">
            @if(session('success'))
            ...
            @endif
            @if($errors->any())
            <div class="bg-red-500/20 border border-red-500/30 text-red-400 px-4 py-3 rounded-lg mb-6 text-sm">
                @foreach($errors->all() as $error)<p>{{ $error }}</p>@endforeach
            </div>
            @endif

            <div class="mb-6">
                <h1 class="text-2xl lg:text-3xl font-bold text-white">Manage Crypto Assets</h1>
                <p class="text-gray-400 text-sm mt-1">Fund or adjust user crypto balances across all supported assets.</p>
            </div>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-5">
                ...
                <div class="xl:col-span-1 order-1">
                    <div class="bg-gray-800 rounded-xl border border-gray-700 p-5 xl:sticky xl:top-20">
                        ...
                        <form method="POST" action="{{ route('admin.manage-crypto.fund') }}" class="space-y-4">
                            ...
                            <div x-show="selectedUserId && selectedSymbol" x-cloak>
                                <div class="bg-gray-700/40 rounded-lg p-3 border border-gray-600">
                                    ...
                                    <p class="text-white font-mono font-semibold text-sm" x-text="currentBalance + ' ' + selectedSymbol"></p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-3 gap-2">
                                    <label class="cursor-pointer">
                                        ...
                                        <div class="text-center py-2 rounded-lg border border-gray-600 text-gray-400 text-xs font-semibold peer-checked:border-green-500 peer-checked:text-green-400 peer-checked:bg-green-500/10 transition">Add</div>
                                    </label>
                                    <label class="cursor-pointer">
                                        <input type="radio" name="action" value="subtract" class="sr-only peer">
                                        <div class="text-center py-2 rounded-lg border border-gray-600 text-gray-400 text-xs font-semibold peer-checked:border-red-500 peer-checked:text-red-400 peer-checked:bg-red-500/10 transition">Subtract</div>
                                    </label>
                                    <label class="cursor-pointer">
                                        <input type="radio" name="action" value="set" class="sr-only peer">
                                        <div class="text-center py-2 rounded-lg border border-gray-600 text-gray-400 text-xs font-semibold peer-checked:border-orange peer-checked:text-orange peer-checked:bg-orange/10 transition">Set</div>
                                    </label>
                                </div>
                            </div>
                            ...
                            <button type="submit"
                                class="w-full bg-orange hover:bg-orange-600 text-white font-semibold py-3 px-4 rounded-xl transition-all active:scale-95 shadow-lg">
                                Apply Balance
                            </button>
                        </form>
                    </div>
                ...
                <div class="xl:col-span-2 order-2">
                    <div class="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
                        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-700">
                            <h2 class="text-base font-semibold text-white">User Balances</h2>
                            ...
                            <table class="w-full">
                                <thead>
                                    <tr class="border-b border-gray-700 bg-gray-900/40">
                                        ...
                                        <th class="text-left py-3 px-5 text-gray-400 text-xs font-semibold uppercase">Balances</th>
                                ...
                                <tbody class="divide-y divide-gray-700/50">
                                    ...
                                    <tr class="hover:bg-gray-700/20 transition-colors">
                                        <td class="py-4 px-5">
                                            <div class="flex items-center gap-3">
                                                <div class="w-9 h-9 bg-navy rounded-full flex items-center justify-center shrink-0">
                                                    <span class="text-white text-xs font-bold">{{ $user->initials }}</span>
                                                </div>
                                                <div class="min-w-0">
                                                    <p class="text-white text-sm font-medium truncate max-w-[130px] xl:max-w-none">{{ $user->name }}</p>
                                                    <p class="text-gray-500 text-xs truncate max-w-[130px] xl:max-w-none">{{ $user->email }}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="py-4 px-5">
                                            ...
                                            <div class="flex flex-wrap gap-1.5">
                                                @foreach($balances as $sym => $amt)
                                                    @if($amt > 0)
                                                    <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold bg-gray-700 border border-gray-600">
                                        ...
                                        <td class="py-4 px-5">
                                            <a href="{{ route('admin.users.view', $user->id) }}" class="text-indigo-400 hover:text-indigo-300 transition text-xs font-medium whitespace-nowrap">View User</a>
                                        </td>
                                    </tr>
                                    @empty
                                    <tr><td colspan="3" class="py-10 text-center text-gray-500">No users found.</td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
                        ...
                        <!-- Mobile Cards -->
                        <div class="sm:hidden divide-y divide-gray-700/50">
                            @forelse($users as $user)
                            <div class="p-4">
                                <div class="flex items-center gap-3 mb-3">
                                    ...
                                    <a href="{{ route('admin.users.view', $user->id) }}" class="text-indigo-400 text-xs shrink-0">View</a>
                                </div>
                                @php $balances = $user->crypto_balances ?? []; @endphp
                                @if(count(array_filter($balances, fn($v) => $v > 0)) > 0)
                                <div class="flex flex-wrap gap-1.5">
                                ...
                                @else
                                <p class="text-gray-600 text-xs">No balances</p>
                                @endif
                            </div>
                            @empty
                            <div class="p-10 text-center text-gray-500">No users found.</div>
                            @endforelse
                        </div>
                    </div>
                </div>
        ...
        function cryptoManager() {
            return {
                ...
                updateUserBalances() {
                    ...
                    try { this.userBalances = JSON.parse(option?.dataset?.balances || '{}') || {}; } catch { this.userBalances = {}; }
                    this.updateCurrentBalance();
                },
                updateCurrentBalance() {
                    const bal = this.userBalances[this.selectedSymbol.toUpperCase()];
